...
router.get('/', (req, res, next) => {
    ...
    Pokemon
        .countDocuments(condition) // 필터링된 문서 수를 가져옴
        .then(totalCount => {
            Pokemon.aggregate([
                { $match: condition }, // 매칭 조건 적용
                { $unwind: "$dex" }, // dex 배열의 각 요소를 분리
                { $match: { "dex.region": region } }, // 특정 region에 해당하는 dex만 선택
                { $sort: { "dex.num": 1 } }, // num으로 정렬
                { $addFields: { totalCount: totalCount } }, // totalCount 필드에 총 갯수 할당
